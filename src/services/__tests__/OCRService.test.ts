...
describe('OCRService', () => {
  ...
  describe('processBatch', () => {
    ...
    it('should continue processing if one image fails', async () => {
      // Mock manipulateAsync to fail on second image
      const { manipulateAsync } = require('expo-image-manipulator');
      let callCount = 0;
      manipulateAsync.mockImplementation(() => {
        ...
        return Promise.resolve({ uri: 'processed-uri' });
      });

      const results = await ocrService.processBatch(['image1.jpg', 'image2.jpg', 'image3.jpg']);
      
      // Should continue with remaining images even if one fails
